...
export function EstateForm({ onSubmissionSuccess }: EstateFormProps) {
  ...
  async function onSubmit(values: EstateFormValues) {
    ...
    if (result.success) {
      setSubmissionStatus({
        type: 'success',
        message: result.message || 'Estate submitted successfully for review!',
      })
      reset()
      setLgas([])
      setCities([])
          ...
          >
            <NextUICardBody className='p-4'>
              <div className='flex items-center'>
                ...
                <div>
                  <p
                    className={`font-semibold ${submissionStatus.type === 'success' ? 'text-success-700' : 'text-danger-700'}`}
                  >
                    {submissionStatus.type === 'success' ? 'Success' : 'Error'}
                  </p>
                  <p
                    className={`text-sm ${submissionStatus.type === 'success' ? 'text-success-600' : 'text-danger-600'}`}
                  >
            ...
            <Controller
              ...
              render={({ field }) => (
                <NextUISelect
                  ...
                  errorMessage={
                    errors.district?.message || errors.city?.message
                  }
            ...
            <Controller
              ...
              render={({ field }) => (
                <NextUISelect
                  ...
                  errorMessage={
                    errors.city?.message || errors.district?.message
                  }
                  isLoading={isLoadingCities}
                  isDisabled={!watchedLgaName || cities.length === 0}
                  selectedKeys={field.value ? [field.value] : []}
