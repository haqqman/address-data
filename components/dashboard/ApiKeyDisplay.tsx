...
export function ApiKeyDisplay() {
    ...
    try {
      ...
      if (result.success && result.apiKey && result.apiKey.privateKey) {
        setNewlyGeneratedKey({
          ...
          privateKey: result.apiKey.privateKey,
        })
  ...
  const handleRevokeKey = async (keyId: string) => {
    if (
      !confirm(
        'Are you sure you want to revoke this API key? This action cannot be undone directly, though it might be reactivatable by an admin.',
      )
    ) {
      return
    }
    ...
    <>
        ...
        <CardBody className='space-y-6 p-6'>
          ...
          <div className='flex items-start p-4 bg-danger-50 border border-danger-200 rounded-lg text-danger-700'>
            ...
            <div>
              ...
              <p className='text-sm'>
                Your Private Key is shown only once upon generation. Treat it
                like a password and store it securely. If lost, you will need to
            ...
            <Popover placement='top'>
              <PopoverTrigger>
                <Button
                  ...
                  startContent={
                    !isGenerating ? <PlusCircle className='h-4 w-4' /> : null
                  }
                >
      ...
      <Card className='shadow-lg rounded-xl bg-background'>
        ...
        <CardBody className='p-6'>
          ...
          {!isLoadingKeys && error && (
            <p className='text-danger-500 text-center py-4'>{error}</p>
          )}
