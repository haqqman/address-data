...
export function FlaggedAddressTable({
  addresses,
  onActionComplete,
}: FlaggedAddressTableProps) {
  const {
  ...
  const handleAction = async (newStatus: 'approved' | 'rejected') => {
    if (!selectedSubmissionId || !user || user.role !== 'administrator') {
      console.error(
        'Action cannot be performed. Console user not found or invalid submission.',
      )
      return
    }

    const result = await updateAddressStatus(
    ...
    setReviewNotes('')
    if (newStatus === 'approved') onApproveClose()
    else onRejectClose()
  ...
  if (addresses.length === 0) {
    return (
      <p className='text-foreground-500'>
        No addresses are currently pending review.
      </p>
    )
  }
                  ...
                  {submission.aiFlaggedReason ? (
                    ...
                    >
                      <span
                        ...
                        title={submission.aiFlaggedReason}
                      >
                        {submission.aiFlaggedReason}
                      </span>
                    </NextUIChip>
