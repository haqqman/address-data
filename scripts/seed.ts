...
import type { User, AddressSubmission, APIKey, FirestoreGeographyStateData } from '@/types'
...
const consoleUsersData: Array<Pick<User, 'id' | 'email' | 'firstName' | 'lastName' | 'role' | 'phoneNumber'>> = [
  {
    id: 'PjOUhbonSTYTA6HPUqHUy0AER6f2', 
  ...
  {
    id: '3aolsEOxNpT8tunbxP2TzqH3RAx2', 
...
async function seedConsoleUserProfiles(batch: ReturnType<typeof writeBatch>): Promise<number> {
  ...
  for (const userData of consoleUsersData) {
    ...
    const profileData: Partial<User> & { createdAt?: any, lastLogin?: any, authProvider?: string, name?: string } = {
      ...
      role: userData.role,
      phoneNumber: userData.phoneNumber || null, // Ensure phoneNumber is null if undefined
...
async function seedGeographyData(batch: ReturnType<typeof writeBatch>): Promise<number> {
  ...
  for (const stateOpt of geographyStateOptions) {
    ...
    if (stateOpt.value === "abuja") capital = "Abuja"; 
    if (stateOpt.value === "lagos") capital = "Ikeja"; 

    const stateDataToSet: FirestoreGeographyStateData = {
      name: stateOpt.title,
      capital: capital 
    }
    batch.set(stateRef, stateDataToSet)
    console.log(`→ Seed State: ${stateOpt.title} (Capital: ${capital})`)
    count++
    ...
    if (stateDetails) {
      for (const lga of stateDetails.lgas) {
        ...
        batch.set(lgaRef, { name: lga.name, stateId: stateId }) 
        ...
        for (const city of lga.cities) {
          ...
          batch.set(cityRef, { name: city.name, lgaId: lgaId, stateId: stateId })
...
const mockAddressSubmissions: Array<Omit<AddressSubmission, 'id' | 'submittedAt' | 'reviewedAt' | 'userName' | 'userEmail' | 'reviewerId' | 'reviewNotes' > & { reviewerId?: string | null, reviewNotes?: string | null}> = [
  {
    userId: 'PjOUhbonSTYTA6HPUqHUy0AER6f2', 
    submittedAddress: {
    ...
    status: 'approved',
    reviewerId: '3aolsEOxNpT8tunbxP2TzqH3RAx2', 
    reviewNotes: 'Looks good, verified via satellite.'
  },
  {
    userId: '3aolsEOxNpT8tunbxP2TzqH3RAx2', 
    submittedAddress: {
  ...
  {
    userId: 'PORTAL_USER_EXAMPLE_UID_1', 
    submittedAddress: {
...
async function seedAddressSubmissions(batch: ReturnType<typeof writeBatch>): Promise<number> {
  ...
  for (const submissionData of mockAddressSubmissions) {
    ...
    if (!userName && submissionData.userId === 'PORTAL_USER_EXAMPLE_UID_1') { 
        userName = "Portal User One";
        userEmail = "portaluser1@example.com";
    }


    const fullSubmission: Partial<AddressSubmission> & {submittedAt: any, reviewedAt: any} = {
      ...submissionData,
      userName: userName || "Unknown User",
      userEmail: userEmail || "unknown@example.com",
      submittedAt: Timestamp.fromDate(new Date(Date.now() - Math.random() * 1000 * 60 * 60 * 24 * 30)), 
...
const mockApiKeys: Array<Omit<APIKey, 'id' | 'createdAt' | 'lastUsedAt' | 'userName' | 'userEmail'>> = [
  {
    ...
    privateKeyHash: 'hashed_pk_live_example_dev_one_key123', 
    isActive: true,
    name: "Dev One's Main App Key"
  },
  {
    userId: 'PORTAL_USER_EXAMPLE_UID_2', 
...
async function seedApiKeys(batch: ReturnType<typeof writeBatch>): Promise<number> {
  ...
  for (const apiKeyData of mockApiKeys) {
    ...
    const fullApiKeyData: APIKey & {createdAt: any, lastUsedAt: any} = {
      id: ref.id, 
      ...apiKeyData,
      userName: userName || "API User",
      userEmail: userEmail || "apiuser@example.com",
      createdAt: Timestamp.fromDate(new Date(Date.now() - Math.random() * 1000 * 60 * 60 * 24 * 90)), 
