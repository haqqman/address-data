...
4.  If the `hostname` _is_ the console hostname but the `pathname` does _not_ start with `/console` (and is not an internal Next.js or API asset), rewrite that to a `404 Not Found` page as well.
