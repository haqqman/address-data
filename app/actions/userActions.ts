...
const PROTECTED_UIDS = ['DOUKechRV9NoSkNpgGL2jNCp6Sz2']
...
const consoleUserCreateSchema = z.object({
  email: z
    ...
    .refine(
      ...
      'Email must end with @haqqman.com',
    ),
  password: z.string().min(6, 'Password must be at least 6 characters.'),
  firstName: z.string().min(1, 'First name is required.'),
  lastName: z.string().min(1, 'Last name is required.'),
  phoneNumber: z.string().optional(),
  role: z.enum(['cto', 'administrator', 'manager'], {
...
export type ConsoleUserUpdateFormValues = z.infer<
  typeof consoleUserUpdateSchema
>
...
export async function getConsoleUsers(): Promise<User[]> {
  try {
    ...
    snapshot.forEach((doc) => {
      users.push({
        id: doc.id,
        ...convertAdminUserTimestamps(doc.data()),
      } as User)
    })
    return users
  } catch (error) {
...
export async function updateConsoleUser(
  ...
  try {
    ...
    if (!validation.success) {
      ...
      return {
        success: false,
        message: `Invalid data provided: ${errorMessages}`,
      }
    }
    ...
    if (PROTECTED_UIDS.includes(uid)) {
      return {
        success: false,
        message: `This is a protected user account and cannot be modified.`,
      }
    }
    ...
    return {
      success: true,
      message: `Successfully updated details for ${dataToUpdate.displayName}.`,
    }
  } catch (error) {
...
export async function deleteConsoleUser(
  uid: string,
): Promise<{ success: boolean; message: string }> {
  try {
    // Verify admin access
    ...
    if (PROTECTED_UIDS.includes(uid)) {
      return {
        success: false,
        message: 'This is a protected user and cannot be deleted.',
      }
    }
    ...
    return {
      success: true,
      message:
        'Console user profile deleted successfully. The authentication record still exists.',
    }
  } catch (error) {
    ...
    return {
      success: false,
      message: `Failed to delete user profile: ${errorMessage}`,
    }
  }
}

export async function getPortalUsers(): Promise<
