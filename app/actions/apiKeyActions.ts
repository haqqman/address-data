...
export async function createApiKey({ 
  ...
  try {
    ...
    if (userId && userId !== sessionUser.id) {
        ...
        if (!isConsole) {
             return { success: false, message: "Permission denied. Only admins can create keys for others." };
        }
        targetUserId = userId;
        ...
        try {
            ...
            console.warn("Failed to fetch target user details for API key creation", e);
        }
    }
