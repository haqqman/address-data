...
export default function GeographyManagementPage() {
  ...
  const [selectedStateIdForLgas, setSelectedStateIdForLgas] = useState<
    string | null
  >(null)
  const [selectedStateIdForCities, setSelectedStateIdForCities] = useState<
    string | null
  >(null)
  const [selectedLgaIdForCities, setSelectedLgaIdForCities] = useState<
    string | null
  >(null)
  ...
  const [selectedStateForNewLga, setSelectedStateForNewLga] =
    useState<string>('')
  const [newCityName, setNewCityName] = useState('')
  const [selectedStateForNewCity, setSelectedStateForNewCity] =
    useState<string>('')
  const [selectedLgaForNewCity, setSelectedLgaForNewCity] = useState<string>('')
  const [lgasForCityDropdown, setLgasForCityDropdown] = useState<
    GeographyLGA[]
  >([])

  const [editingEntity, setEditingEntity] = useState<EditableGeoEntity | null>(
    null,
  )
  ...
  const [deletingEntity, setDeletingEntity] =
    useState<EditableGeoEntity | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [activeGeoTab, setActiveGeoTab] = useState<string | number>('states')

  const isManagingFCTDistricts =
    states.find((s) => s.id === selectedStateIdForCities)?.name === 'FCT' &&
    lgas.find((l) => l.id === selectedLgaIdForCities)?.name ===
  ...
  useEffect(() => {
    if (
      ...
      const fetchLgasForDropdown = async () => {
        ...
        const fctState = states.find(
          (s) => s.id === selectedStateForNewCity && s.name === 'FCT',
        )
        if (fctState) {
          const amacLga = fetchedLgas.find(
            (l) => l.name === 'Municipal Area Council',
          )
          if (amacLga) {
  ...
  const handleAddEntity = async () => {
    ...
    try {
      if (entityTypeToAdd === 'State') {
        ...
        if (selectedStateIdForLgas === selectedStateForNewLga)
          fetchLgas(selectedStateForNewLga)
      } else if (entityTypeToAdd === 'City' || entityTypeToAdd === 'District') {
        if (
          !newCityName.trim() ||
          !selectedStateForNewCity ||
          !selectedLgaForNewCity
        ) {
          alert('Name, parent state, and parent LGA are required.')
          setIsSubmitting(false)
          return
        }
        await addCity(selectedStateForNewCity, selectedLgaForNewCity, {
          name: newCityName,
        })
        setNewCityName('')
        ...
        if (
          selectedStateIdForCities === selectedStateForNewCity &&
          selectedLgaIdForCities === selectedLgaForNewCity
        )
          fetchCities(selectedStateForNewCity, selectedLgaForNewCity)
      }
      onAddModalClose()
    } catch (error) {
  ...
  const openEditModal = (
    entity: GeographyState | GeographyLGA | GeographyCity,
    type: GeoEntityType,
  ) => {
    ...
    setEditingEntity({
      ...
      name: entity.name,
      capital:
        type === 'State' ? (entity as GeographyState).capital : undefined,
      type: type,
      parentId: parentId,
    ...
    if (type === 'State')
      setEditedCapital((entity as GeographyState).capital || '')
    onEditModalOpen()
  }
  ...
  const handleEditEntity = async () => {
    ...
    try {
      if (editingEntity.type === 'State') {
        await updateState(editingEntity.id, {
          name: editedName,
          capital: editedCapital,
        })
        fetchStates()
      } else if (editingEntity.type === 'LGA' && editingEntity.parentId) {
        await updateLga(editingEntity.parentId, editingEntity.id, {
          name: editedName,
        })
        if (selectedStateIdForLgas === editingEntity.parentId)
          fetchLgas(editingEntity.parentId)
      } else if (
        (editingEntity.type === 'City' || editingEntity.type === 'District') &&
        editingEntity.grandParentId &&
        editingEntity.parentId
      ) {
        await updateCity(
          editingEntity.grandParentId,
          editingEntity.parentId,
        ...
        if (
          selectedStateIdForCities === editingEntity.grandParentId &&
          selectedLgaIdForCities === editingEntity.parentId
        )
          fetchCities(editingEntity.grandParentId, editingEntity.parentId)
      }
      onEditModalClose()
    } catch (error) {
  ...
  const handleDeleteEntity = async () => {
    ...
    try {
      if (deletingEntity.type === 'State') {
        ...
        fetchStates()
        if (selectedStateIdForLgas === deletingEntity.id)
          setSelectedStateIdForLgas(null)
        if (selectedStateIdForCities === deletingEntity.id)
          setSelectedStateIdForCities(null)
      } else if (deletingEntity.type === 'LGA' && deletingEntity.parentId) {
        await deleteLga(deletingEntity.parentId, deletingEntity.id)
        if (selectedStateIdForLgas === deletingEntity.parentId)
          fetchLgas(deletingEntity.parentId)
        if (selectedLgaIdForCities === deletingEntity.id)
          setSelectedLgaIdForCities(null)
      } else if (
        (deletingEntity.type === 'City' ||
          deletingEntity.type === 'District') &&
        deletingEntity.grandParentId &&
        deletingEntity.parentId
      ) {
        await deleteCity(
          deletingEntity.grandParentId,
          deletingEntity.parentId,
        ...
        if (
          selectedStateIdForCities === deletingEntity.grandParentId &&
          selectedLgaIdForCities === deletingEntity.parentId
        )
          fetchCities(deletingEntity.grandParentId, deletingEntity.parentId)
      }
      onDeleteModalClose()
    } catch (error) {
      console.error('Error deleting entity:', error)
      alert(
        `Failed to delete ${deletingEntity.type}. It might have child elements that need to be removed first.`,
      )
    } finally {
      setIsSubmitting(false)
      ...
      >
        <Tab
          ...
          title={
            <span className='flex items-center'>
              <Map
                className={`mr-2 h-5 w-5 ${activeGeoTab === 'states' ? 'text-warning' : 'text-secondary'}`}
              />
              States
            </span>
          }
              ...
              {isLoadingStates ? (
                ...
                <ul className='space-y-2'>
                  {states.map((state) => (
                    <li
                      key={state.id}
                      className='p-3 border rounded-lg flex justify-between items-center hover:bg-default-100'
                    >
                      <div>
        ...
        <Tab
          ...
          title={
            <span className='flex items-center'>
              <Building
                className={`mr-2 h-5 w-5 ${activeGeoTab === 'lgas' ? 'text-warning' : 'text-secondary'}`}
              />
              LGAs
            </span>
          }
          ...
          <NextUICard className='shadow-xl rounded-xl bg-background'>
            <NextUICardHeader className='px-6 pt-6 pb-2'>
              <div className='flex justify-between items-center w-full mb-4'>
                ...
                <NextUISelect
                  ...
                  selectedKeys={
                    selectedStateIdForLgas ? [selectedStateIdForLgas] : []
                  }
                  onSelectionChange={(keys) =>
                    setSelectedStateIdForLgas(Array.from(keys)[0] as string)
                  }
                  className='min-w-64'
                  variant='bordered'
                  color='secondary'
            ...
            <NextUICardBody className='p-2 md:p-4'>
              {isLoadingLgas ? (
                ...
                <p className='text-foreground-500'>Please select a state.</p>
              ) : lgas.length === 0 && selectedStateIdForLgas ? (
                <p className='text-foreground-500'>
        ...
        <Tab
          ...
          title={
            <span className='flex items-center'>
              <MapPin
                className={`mr-2 h-5 w-5 ${activeGeoTab === 'cities' ? 'text-warning' : 'text-secondary'}`}
              />
              Cities & Districts
            </span>
          }
          ...
          <NextUICard className='shadow-xl rounded-xl bg-background'>
            <NextUICardHeader className='px-6 pt-6 pb-2'>
              <div className='flex justify-between items-start w-full mb-4'>
                <div className='flex flex-col space-y-0.5'>
                  ...
                  <p className='text-sm text-foreground-500'>
                    Select a state and LGA to manage its cities or districts.
                  </p>
                </div>
                <div className='flex gap-4'>
                  <NextUISelect
                    ...
                    placeholder='Choose a state'
                    selectedKeys={
                      selectedStateIdForCities ? [selectedStateIdForCities] : []
                    }
                    onSelectionChange={(keys) => {
                  ...
                  <NextUISelect
                    ...
                    selectedKeys={
                      selectedLgaIdForCities ? [selectedLgaIdForCities] : []
                    }
                    onSelectionChange={(keys) =>
                      setSelectedLgaIdForCities(Array.from(keys)[0] as string)
                    }
                    className='min-w-64'
            ...
            <NextUICardBody className='p-2 md:p-4'>
              {isLoadingCities ? (
                ...
                <p className='text-foreground-500'>
                  Please select a state and LGA.
                </p>
              ) : cities.length === 0 && selectedLgaIdForCities ? (
                <p className='text-foreground-500'>
                  No {isManagingFCTDistricts ? 'districts' : 'cities'} found for
                  the selected LGA.
                </p>
              ) : (
                <ul className='space-y-2'>
                      ...
                      <div className='space-x-1'>
                        <NextUIButton
                          ...
                          onPress={() =>
                            openEditModal(
                              city,
                              isManagingFCTDistricts ? 'District' : 'City',
                            )
                          }
                        >
                          <Edit3 className='h-4 w-4' />
            ...
            <>
              ...
              <ModalBody>
                <NextUISelect
                  ...
                  onSelectionChange={(keys) =>
                    setEntityTypeToAdd(Array.from(keys)[0] as GeoEntityType)
                  }
                  variant='bordered'
                  color='secondary'
                >
                ...
                {entityTypeToAdd === 'State' && (
                  <>
                    ...
                    <NextUIInput
                      ...
                      onValueChange={setNewStateCapital}
                      variant='bordered'
                    />
                  </>
                )}
                {entityTypeToAdd === 'LGA' && (
                  <>
                    <NextUISelect
                      ...
                      selectedKeys={
                        selectedStateForNewLga ? [selectedStateForNewLga] : []
                      }
                      onSelectionChange={(keys) =>
                        setSelectedStateForNewLga(Array.from(keys)[0] as string)
                      }
                      variant='bordered'
                      color='secondary'
                    >
                ...
                {(entityTypeToAdd === 'City' ||
                  ...
                  <>
                    <NextUISelect
                      ...
                      placeholder='Select a state'
                      selectedKeys={
                        selectedStateForNewCity ? [selectedStateForNewCity] : []
                      }
                      onSelectionChange={(keys) => {
                        const stateId = Array.from(keys)[0] as string
                    ...
                    <NextUISelect
                      ...
                      selectedKeys={
                        selectedLgaForNewCity ? [selectedLgaForNewCity] : []
                      }
                      onSelectionChange={(keys) =>
                        setSelectedLgaForNewCity(Array.from(keys)[0] as string)
                      }
                      isDisabled={
                        !selectedStateForNewCity ||
                        lgasForCityDropdown.length === 0 ||
                        states.find((s) => s.id === selectedStateForNewCity)
                          ?.name === 'FCT'
                      }
                      variant='bordered'
                      color='secondary'
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className='flex flex-col gap-1 text-primary'>
                Delete {deletingEntity?.type}?
              </ModalHeader>
              <ModalBody>
                <p>
                  Are you sure you want to delete{' '}
                  <span className='font-semibold'>{deletingEntity?.name}</span>?
                </p>
                <p className='text-sm text-danger'>
                  This action cannot be undone. If this {deletingEntity?.type}{' '}
                  has child elements (e.g., a State has LGAs), they might also
                  be affected or prevent deletion.
                </p>
              </ModalBody>
              <ModalFooter>
                <NextUIButton
