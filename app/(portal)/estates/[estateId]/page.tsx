...
export default function ManageEstatePage() {
  ...
  const handleUpdate = async () => {
    ...
    if (editForm.googleMapLink !== (estate.googleMapLink || ''))
      dataToUpdate.googleMapLink = editForm.googleMapLink
  ...
  const handleStatusUpdate = async (newStatus: 'verified' | 'rejected') => {
    ...
    if (result.success) {
      setSubmissionStatus({
        type: 'success',
        message: `Estate ${newStatus === 'verified' ? 'verified' : 'rejected'} successfully.`,
      })
      fetchEstate()
    } else {
      setSubmissionStatus({ type: 'error', message: result.message })
    ...
    return (
      <div className='space-y-4'>
        ...
        <Card className='shadow-lg rounded-xl'>
          <CardHeader>
            <Skeleton className='h-8 w-3/5 rounded-lg bg-default-200' />
          </CardHeader>
  ...
  return (
    <div className='space-y-8'>
          ...
          <div>
            <div className='flex items-center gap-2'>
              ...
              <Chip
                ...
                variant='flat'
                color={
                  estate.status === 'verified'
                    ? 'success'
                    : estate.status === 'pending-review'
                      ? 'warning'
                      : 'danger'
                }
              >
            ...
            <p className='text-foreground-500 font-mono text-sm mt-1'>
              {estate.estateCode}
            </p>
          </div>
          {!isEditing && (
            <div className='flex items-center gap-3'>
              {['cto', 'administrator', 'manager'].includes(user?.role || '') &&
                estate.status === 'pending-review' && (
      ...
      {submissionStatus && (
        <Card
          className={`mb-6 ${submissionStatus.type === 'success' ? 'bg-success-50 border-success-200' : 'bg-danger-50 border-danger-200'}`}
        >
          <CardBody className='p-4'>
            <div className='flex items-center'>
              ...
              <p
                className={`text-sm font-medium ${submissionStatus.type === 'success' ? 'text-success-700' : 'text-danger-700'}`}
              >
          ...
          {isEditing ? (
            <div className='space-y-4'>
              ...
              <Input
                ...
                onValueChange={(v) =>
                  setEditForm({ ...editForm, googleMapLink: v })
                }
                variant='bordered'
          ...
          <>
            ...
            <CardFooter className='text-xs text-foreground-500 justify-between'>
              <div>
                ...
                <p>
                  Created By:{' '}
                  {estate.createdBy.startsWith('system')
                    ? 'System'
                    : estate.createdBy}
                </p>
              </div>
              <div>
                ...
                <p>
                  Updated By:{' '}
                  {estate.lastUpdatedBy.startsWith('system')
                    ? 'System'
