...
import { Skeleton, Card, CardBody, Button, Tabs, Tab } from '@heroui/react'
...
export default function AddressesPage() {
  ...
  const [activeFilter, setActiveFilter] =
    useState<FilterType>('my_contributions')
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      ...
      const userIdToFetch =
        activeFilter === 'my_contributions' ? user?.id : undefined
  ...
  const listTitle = useMemo(() => {
    return activeFilter === 'my_contributions'
      ? 'My Contributions'
      : 'All Contributions'
  }, [activeFilter])
  ...
  return (
    <div className='space-y-8'>
      <div className='flex justify-between items-center'>
        <div className='flex flex-col space-y-1'>
          ...
          <p className='text-foreground-500'>
            View and manage address submissions.
          </p>
        </div>
        <Button
          as={Link}
      ...
      {!isLoading && !authLoading && !error && (
        <AddressList title={listTitle} addresses={filteredSubmissions} />
      )}
    </div>
  )
}
